...
if sw_o == 2
  @fastmath @inbounds for i_down = 1:Lmax
    kvv_down[i_down,:] = i_down * (j_up+1) * k10
  end
elseif sw_o == 1
  aA     = a * 1e-10
  mu_amu = mu / 1.6605e-27
  dE     = om_x_e * 1.4388e-2
  delta  = 0.427 / aA * sqrt(mu_amu / t) * dE;
  @fastmath @inbounds for i_down = 1:Lmax
  @. kvv_down[i_down,:] = i_down * (j_up+1) * k10 .* exp.(-delta .* abs(i_down-1-j_up)) .*
